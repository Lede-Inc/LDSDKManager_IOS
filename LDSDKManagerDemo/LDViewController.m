...
-(void)loginByWX
{
    [[LDSDKManager sharedManager] loginFromPlatformType:LDSDKPlatformWeChat withCallback:^(NSDictionary *oauthInfo, NSDictionary *userInfo, NSError *error) {
        if (error==nil) {
...
-(void)loginByQQ
{
    [[LDSDKManager sharedManager] loginFromPlatformType:LDSDKPlatformQQ withCallback:^(NSDictionary *oauthInfo, NSDictionary *userInfo, NSError *error) {
        if (error==nil) {
            if (userInfo==nil && oauthInfo!=nil) {
                [infoLabel setText:@"授权成功"];
            } else {
...
-(void)shareByQQ
{
    ...
    NSDictionary *shareDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               nil];
    [[LDSDKManager sharedManager] shareToPlatform:LDSDKPlatformQQ shareModule:LDSDKShareToContact withDict:shareDict onComplete:^(BOOL success, NSError *error) {
        if (success) {
...
-(void)shareByWX
{
    ...
    NSDictionary *shareDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               @"text", LDShareDictTextKey,
                               nil];
    [[LDSDKManager sharedManager] shareToPlatform:LDSDKPlatformWeChat shareModule:LDSDKShareToContact withDict:shareDict onComplete:^(BOOL success, NSError *error) {
        if (success) {
            [infoLabel setText:@"分享成功"];
        } else {
            [infoLabel setText:error.localizedDescription];
...
-(void)shareByQzone
{
    ...
    NSDictionary *shareDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               @"text", LDShareDictTextKey,
                               nil];
    [[LDSDKManager sharedManager] shareToPlatform:LDSDKPlatformQQ shareModule:LDSDKShareToTimeLine withDict:shareDict onComplete:^(BOOL success, NSError *error) {
        if (success) {
            [infoLabel setText:@"分享成功"];
        } else {
            [infoLabel setText:error.localizedDescription];
...
-(void)shareByWXTimeline
{
    ...
    NSDictionary *shareDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               @"text", LDShareDictTextKey,
                               nil];
    [[LDSDKManager sharedManager] shareToPlatform:LDSDKPlatformWeChat shareModule:LDSDKShareToTimeLine withDict:shareDict onComplete:^(BOOL success, NSError *error) {
        if (success) {
            [infoLabel setText:@"分享成功"];
        } else {
            [infoLabel setText:error.localizedDescription];
...
-(void)shareByYXTimeline
{
    ...
    NSDictionary *shareDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               @"text", LDShareDictTextKey,
                               nil];
    [[LDSDKManager sharedManager] shareToPlatform:LDSDKPlatformYiXin shareModule:LDSDKShareToTimeLine withDict:shareDict onComplete:^(BOOL success, NSError *error) {
        if (success) {
            [infoLabel setText:@"分享成功"];
        } else {
            [infoLabel setText:error.localizedDescription];
...
-(void)shareByYX
{
    ...
    NSDictionary *shareDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               @"text", LDShareDictTextKey,
                               nil];
    [[LDSDKManager sharedManager] shareToPlatform:LDSDKPlatformYiXin shareModule:LDSDKShareToContact withDict:shareDict onComplete:^(BOOL success, NSError *error) {
        if (success) {
            [infoLabel setText:@"分享成功"];
        } else {
            [infoLabel setText:error.localizedDescription];
...
-(void)payByWX
{
    [[LDSDKManager sharedManager] payOrderWithType:LDSDKPlatformWeChat orderString:@"partnerId=1004" callback:^(NSString *signString, NSError *error) {
        if (signString) {
...
-(void)payByAli
{
    [[LDSDKManager sharedManager] payOrderWithType:LDSDKPlatformAliPay orderString:@"" callback:^(NSString *signString, NSError *error) {
